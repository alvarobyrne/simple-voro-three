...
class App {
  ...
  #createDebugPanel() {
    ...
    this.pane
      .addButton({ title: "voro++" })
      .on("click", this.#getOnClick("https://math.lbl.gov/voro++"));
    this.pane
      .addButton({ title: "voro++: examples" })
      .on("click", this.#getOnClick("https://math.lbl.gov/voro++/examples"));
    this.pane
      .addButton({ title: "voro++: custom output" })
      .on(
        "click",
        this.#getOnClick("https://math.lbl.gov/voro++/doc/custom.html")
      );
    //
    return;
    /**
  ...
  async #loadVoronoiData() {
    ...
    const lines = this.#parseEdgesVertices(edgesVertices).filter(
      (x) => x.length > 0
    );
    ...
    const trianglesIndices = indicesCells.map(
      trianglesIndicesFromPolygonsIndices
    );
  ...
  #makePolyhedronModel(model) {
    return model.centroids.map((centroid, i) => {
      const vertices = model.vertices[i].map(
        (vertex) => new Vector3(...vertex)
      );
  ...
  #drawCellPolyhedra(vertices, indices) {
    ...
    const meshes = vertices.map((polyhedronVertices, i) => {
      ...
      return this.#drawCellPolyhedron(
        polyhedronVertices,
        polyhedronIndices,
        faceColors[i % faceColors.length]
      );
    });
    return meshes;
  }
  ...
  #drawCellPolyhedron(vertices, indices, color) {
    ...
    const points = vertices.map(
      (vectorAsArray) => new Vector3(...vectorAsArray)
    );
  ...
  #parseEdgesVertices(vertices) {
    ...
    const lines = facesRaw.map((face) => {
      ...
      const points = pointsString.map((x) =>
        this.#stringLineToPoints(x, IS_SHIFTNG)
      );
      return points;
    });
    return lines;
